...
class ProtoUtils {
    companion object {
        ...
        fun searchCarrier(
            ...
            id: CarrierConfigService.ExtendedCarrierIdentifier,
        ): String? {
            for (carrierMap in carrierList.entryList) {
                    ...
                    when (carrierId.mvnoDataCase) {
                        ...
                        GID -> {
                            id.gid1?.let {
                                ...
                                if (it.length >= gid.length &&
                                    gid.equals(
                                        it.substring(0, gid.length),
                                        ignoreCase = true,
                                    )
                                ) {
                                    Log.i(TAG, "Carrier ${carrierId.mccMnc} matched!")
                        ...
                        ICCID -> {
                            if (id.iccid.startsWith(carrierId.iccid)) {
                                Log.i(TAG, "Carrier ${carrierId.mccMnc} matched!")
                                return carrierMap.canonicalName
                            }
                        }
...
fun CarrierConfig.toBundle(): PersistableBundle {
    ...
    for (config in configList) {
        Log.i(ProtoUtils.TAG, "Adding key ${config.key}")
        when (config.valueCase) {
            STRING_VALUE -> {
                bundle.putString(config.key, config.stringValue)
            }

            INTEGER_VALUE -> {
                bundle.putInt(config.key, config.integerValue)
            }

            LONG_VALUE -> {
                bundle.putLong(config.key, config.longValue)
            }

            BOOLEAN_VALUE -> {
                bundle.putBoolean(config.key, config.booleanValue)
            }

            DOUBLE_VALUE -> {
                bundle.putDouble(config.key, config.doubleValue)
            }

            STRING_ARRAY_VALUE -> {
                bundle.putStringArray(
                    config.key,
                    arrayOf<String>() + config.stringArrayValue.itemList,
                )
            }
            ...
            BUNDLE_VALUE -> {
                bundle.putPersistableBundle(config.key, config.bundleValue.toBundle())
            }
